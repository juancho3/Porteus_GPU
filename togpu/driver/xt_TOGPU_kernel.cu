...
extern "C" void  gpu_free_pinned_mem( void *ptr );
...
static void   *gpu_ptr, *gpu_devptr;
...
void *gpu_alloc_pinned_mem( unsigned long  size )
{
   ...
   // Get first GPU device on system
   devID = 0;
   checkCudaErrors( cudaSetDevice( devID ) );
   checkCudaErrors( cudaGetDeviceProperties(&deviceProp, devID) );
   printf( "GPU Device %d: \"%s\" with compute capability %d.%d\n\n", devID, deviceProp.name, deviceProp.major, deviceProp.minor );

   // Check if 'cudaDeviceMapHost' flag can be set
   ...
   // Enable 'cudaDeviceMapHost' flag
   checkCudaErrors( cudaSetDeviceFlags(cudaDeviceMapHost) );

   // Alloc pinned-memory
   gpu_ptr = NULL;
   checkCudaErrors( cudaHostAlloc(&gpu_ptr, size, cudaHostAllocMapped | cudaHostAllocPortable | cudaHostAllocWriteCombined) );
   if( gpu_ptr == NULL )
   {
   ...
   // Get pointer for device space
   gpu_devptr = NULL;
   checkCudaErrors( cudaHostGetDevicePointer(&gpu_devptr, gpu_ptr, 0) );
   if( gpu_ptr == NULL )
   {
      fprintf( stderr, "Could not get device pointer associated to pinned-memory\n" );
      cudaDeviceReset();
      return NULL;
   } //endif
...
void  gpu_free_pinned_mem( void  *ptr )
{
   cudaFreeHost( ptr );
   cudaDeviceReset();
}
...
/* -------------------------------------------------------------- */
/* Put your favourite CUDA kernel here to process network traffic */
/* -------------------------------------------------------------- */
...
__global__ void aes_lctr_crypt(u32 *rk, int nrounds, u8 *text, u8 *ctr)
{
    u32 s[4];
    u32 t[4];
    u32 *txt = (u32*)(text+32+(16*(threadIdx.x)));
...
/* Use AES for network traffic */

extern "C" int aes_init( void );
extern "C" int aes_test( void );

u32 cipher_key[128];  // 512-bit key
...
int aes_test( void )
{
   int  index, length, threads;
   u8  *ptr;
   volatile u32 *target = (u32 *) gpu_ptr;

   // This is a neverending process
   while( 1 )
   {
      // Wait till a network packet is ready at GPU pinned-memory
      while( *target == 0 );
      index  = *target;
      length = *(target+1);

      // Do encryption/decryption
      threads = (length-32) / 16;  // 16 == AES block size
      ptr  = (u8 *) gpu_devptr;
      ptr += (index *0x1000);      // 0x1000 == NFGPU_SKB_SIZE

      /* Decryption test - RFU
      aes_decrypt_bpt <<<1,threads>>>
      (
         cipher_key,               // Pointer to key
         sizeof(cipher_key)/4+6,   // Number of rounds (depends on key size)
         ptr                       // Pointer to packet
      );
      */

      // Test: Received network packets will be ciphered by AES
      aes_encrypt_bpt <<<1,threads>>>
      (
         cipher_key,               // Pointer to key
         sizeof(cipher_key)/4+6,   // Number of rounds (depends on key size)
         ptr                       // Pointer to packet
